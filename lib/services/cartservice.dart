...
class CartService extends ChangeNotifier {
  ...
  //   double mainTotal = 0;
  //   _items.forEach((CartItem item) {
  //     Pathology itemSubCategory = (item.category as Pathology);
  ...
  Pathology? getCategoryFromCart(Pathology cat) {
    Pathology? subCat = cat;
    if (_items.length > 0 &&
        _items.any((CartItem item) => item.category!.name == cat.name)) {
      CartItem cartItem =
      ...
      if (cartItem != null) {
        subCat = cartItem.category as Pathology?;
      }
    }
